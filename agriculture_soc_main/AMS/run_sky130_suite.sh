...
sed 's/Xcmp (outn outp/Xcmp (outp outn/' "$A/AMS/spice/sky130_cells.scs" > $REPO_ROOT/badpol_cells.scs
sed "s#include \"\$ACCELERATOR_DIR/AMS/spice/sky130_cells.scs\"#include \"$REPO_ROOT/badpol_cells.scs\"#" \
    "$A/AMS/amsd_sky130_all.scs" > $REPO_ROOT/amsd_badpol.scs
...
timeout 2400 make -C nanosoc_tech run_xr TESTNAME=adc_trigger_test ACCELERATOR=yes \
    TOOL_CHAIN=gcc SAR_AMS_INCLUDE=yes XRUN_EXTRA_SRC=$REPO_ROOT/amsd_badpol.scs \
    AMS_ACF="$A/AMS/acf_sky130.scs" EXTRA_DEFINES=+define+SENSING_CHECK \
    > $REPO_ROOT/neg.log 2>&1
L=$REPO_ROOT/nanosoc/simulate/sim/adc_trigger_test/logs/run_adc_trigger_test.log
...
for T in adc_trigger_test adc_autonomous_test fifo_drain_test rtc_time_test pslverr_test sensing_driver_test; do
  ...
  timeout 2400 make -C nanosoc_tech run_xr TESTNAME=$T ACCELERATOR=yes TOOL_CHAIN=gcc \
      ...
      AMS_ACF="$A/AMS/acf_sky130.scs" EXTRA_DEFINES=+define+SENSING_CHECK \
      > $REPO_ROOT/sky_$T.log 2>&1
  L=$REPO_ROOT/nanosoc/simulate/sim/$T/logs/run_$T.log
